...
from sqlalchemy.databases.postgres import PGDate, PGString, PGChar, PGText, \
        PGInteger, PGBigInteger, PGNumeric, PGFloat, PGBoolean, PGTime, PGBinary
...
class Neptuno(object):
    ...
    def get_tabla_relacionada(self, tabla):
        try:
            return Table('vista_busqueda_%s' % tabla, self.meta, autoload=True)
            
        except NoSuchTableError:
            return Table(tabla, self.meta, autoload=True)
                        
    def get_titulo(self, registro, columna):
        
        # TITULO
        c_name = columna[NOMBRE]
        columna[TITULO] = ''
        
        if DEFINICION_CLASES.has_key(columna[TABLA_RELACIONADA]) and registro:
            titulo = DEFINICION_CLASES[columna[TABLA_RELACIONADA]].get(TITULO)
            if titulo:
                if inspect.isclass(titulo):
                    columna[TITULO] = \
                        titulo.__title__(self.conector, registro[c_name])
                    ...
                    if registro[c_name]:
                        ...
                        tabla_relacionada = self.get_tabla_relacionada(columna[TABLA_RELACIONADA])
                        
                        registro_relacionado = \
                            self.conector.conexion.\
                            execute(tabla_relacionada.\
                                    select(whereclause=tabla_relacionada.c.id == registro[c_name])).\
                            fetchone()
                            
                        # calcular el título
                        titulo = [str(registro_relacionado[campo_r] or '')
                ...
                if registro[c_name]:
                    ...
                    registro_relacionado = \
                        self.conector.conexion.\
                            execute(tabla_relacionada.\
                                    select(tabla_relacionada.c.id == registro[c_name])).\
                            fetchone()
                            
                    if registro_relacionado is None:
                        # el registro relacionado no aparece en la "vista"
                        # aunque existe en la "tabla"
                        tabla_relacionada = Table(columna[TABLA_RELACIONADA],
                    ...
                    for col_r in tabla_relacionada.columns: 
                        if not col_r.name.startswith('id'):                                                        
                            columna[TITULO] = str(registro_relacionado[col_r.name] or '').strip()
                            break
                        
    def get_campos_referencia(self, registro, columna):
        
        # CAMPOS_REFERENCIA
        #if DEFINICION_CLASES.has_key(columna[TABLA_RELACIONADA]) and registro:        
        campos_ref = DEFINICION_CLASES[columna[TABLA_RELACIONADA]].get(TITULO)
        if campos_ref:
    ...
    def get_valores(self, registro, columna):
        ...
        for fila in tabla_busqueda.select().execute():                                        
            fila_resultado = tbl_relacionada.\
                query_registro(id_reg=fila['id'], seguir_foraneas=False)
    ...
    def get_tipo(self, c, columna):
        if isinstance(c.type, PGDate):
            columna[TIPO] = TYPE_DATE
        elif isinstance(c.type, PGTime):
            columna[TIPO] = TYPE_TIME
        elif isinstance(c.type, PGBoolean):
            columna[TIPO] = TYPE_BOOLEAN
        elif isinstance(c.type, PGString):
            columna[TIPO] = TYPE_STRING
        elif isinstance(c.type, PGChar):
            columna[TIPO] = TYPE_CHAR
        elif isinstance(c.type, PGText):
            columna[TIPO] = TYPE_TEXT
        elif isinstance(c.type, PGInteger) or isinstance(c.type, PGBigInteger):
            columna[TIPO] = TYPE_INTEGER
        elif isinstance(c.type, PGNumeric) or isinstance(c.type, PGFloat):
    ...
    def query_registro(self, id_reg=-1, seguir_foraneas=True, **kwargs):
        ...
        for c in self.la_tabla.columns:
            ...
            (c.name != 'busqueda') and (type(c.type) != PGBinary):
                columna = {}
                ...
                if DEFINICION_CLASES != None and c.name in self.campos:
                    ...
                    # campos "ocultos"
                    if el_campo[POS_ETIQUETA] is None:
                        continue
                ...
                try:
                    ...
                    clave_foranea = [k for k in self.la_tabla.foreign_keys 
                                     if k.parent.name == c.name]
                    ...
                    if clave_foranea != [] and seguir_foraneas:
                        ...
                        DEFINICION_CLASES[columna[TABLA_RELACIONADA]][UTILIZAR_SELECTOR]:
                            ...
                            # TITULO
                            self.get_titulo(registro, columna)
                            
                            # CAMPOS REFERENCIA
                            self.get_campos_referencia(registro, columna)

                        else:
                            # VALORES
                            self.get_valores(registro, columna)
