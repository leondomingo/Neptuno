...
from nucleo.const_datos import SESSION_LIFE #, ROLES
from libpy.log import NeptunoLogger
logger = NeptunoLogger.get_logger('libpy.postgres2.unidadescompartidasNeptuno')
...
class SesionesNeptuno(Base):
    ...
    id_usuarios_usuario = Column(Integer, ForeignKey('usuarios.id'))    
...
#        
#        if not conector.conexion.query(usuariosNeptuno).\
#                filter(usuariosNeptuno.nombre_usuario==login).\
#                first() is None:
